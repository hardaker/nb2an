...
def main():
    ...
    interface_addresses = b.get_addresses()
    interfaces = b.get_interfaces()
    ...
    for device in sorted(r['results'], key=lambda x: x['display']):
        ...
        for interface in dev_interfaces:
            ...
            other_end = ""
            if 'cable_peer' in interface and interface['cable_peer'] and 'device' in interface['cable_peer']:
            ...
            print(f"  {interface['name']:<12} {other_end:<10} {ipv4:<20} {ipv6:<30} {interface['type']['label']}")
